...
int heartbeat;
int SPO2, concentration;
// int heartbeat_avg, concentration_avg = 0;
int voltage;
  ...
  do {
    switch (msg) {
      ...
      case 1:
        ...
        // break;
      case 2:
        print_digit(86, 0, heartbeat);
        wave.draw(8);
        print_digit(98, 16, concentration, ' ', 3, 1);
        ...
        break;
      case 3:
        oled.drawStr(30, 9, F("Heart Rate"), 1);
...
void loop() {
  ...
  if (irValue < 5000) {
    ...
    if (draw_Red ? beatRed : beatIR) {
      ...
      if (btpm > 0 && btpm < 200) heartbeat = bpm.filter((int16_t)btpm);
      lastBeat = now;
      ...
      // using formula
      concentration = (10400 - RX100 * 17 + 50) / 100;
      // from table
  ...
  sim800l.write(26);
    ...
    if (now - displaytime >= 1000) {
      ...
      draw_oled(2);
      // i += 1;
      // heartbeat_avg += heartbeat;
      // concentration_avg += concentration;
      // if (i == 18) {
        // heartbeat_avg = int(heartbeat_avg / 18);
        // concentration_avg = int(concentration_avg / 18);
